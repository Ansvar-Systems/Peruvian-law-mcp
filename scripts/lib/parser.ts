/**
 * HTML parser for Peruvian legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Peruvian legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Peruvian statute.
 *
...
export function parsePeruvianHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Peruvian acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Peruvian legal text.
 *
 * Peruvian definitions typically use patterns like:
...
/**
 * Pre-configured list of key Peruvian Acts to ingest.
 *
 * Source: api.sejm.gov.pl (Sejm ELI API)
 * URL pattern: https://api.sejm.gov.pl/eli/acts/DU/{YEAR}/{POZ}/text.html
 *
 * These are the most important Peruvian statutes for cybersecurity, data protection,
