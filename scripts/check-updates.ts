#!/usr/bin/env tsx
/**
 * Peruvian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://spij.minjus.gob.pe';
const PORTAL_NAME = 'SPIJ (Sistema Peruano de Informacion Juridica)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/peruvian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Peruvian Law MCP — Data Freshness Check');
