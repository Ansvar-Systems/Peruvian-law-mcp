/**
 * get_peruvian_implementations — Find Peruvian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetPeruvianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface PeruvianImplementationResult {
  document_id: string;
...
export async function getPeruvianImplementations(
  db: InstanceType<typeof Database>,
  input: GetPeruvianImplementationsInput,
): Promise<ToolResponse<PeruvianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as PeruvianImplementationResult[];
