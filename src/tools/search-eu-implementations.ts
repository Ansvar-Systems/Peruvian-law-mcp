/**
 * search_eu_implementations — Search EU directives/regulations referenced by Peruvian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_peruvian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as peruvian_statute_count
  ...
  if (input.has_peruvian_implementation) {
    sql += ' HAVING peruvian_statute_count > 0';
  }
